...
namespace OrbitalBlitz.Game.Scenes.Circuits.Scripts.CircuitGeneration {
    public class GridPathGenerator : MonoBehaviour, IPathGenerator {
        ...
        public GridPathGenerator SetMode(GenerationMode mode) {
            _mode = mode;
            return this;
        }

        public GridPathGenerator(int2 dimensions, int seed = 123, GenerationMode mode = GenerationMode.Backtracking) {
            m_dims = dimensions;
            m_rd = new(seed);
        }
    ...
    #if UNITY_EDITOR
    [CustomEditor(typeof(GridPathGenerator))]
